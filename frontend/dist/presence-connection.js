// src/utils/presenceConnection.ts
function createPresenceHook(R, url) {
  return function usePresence({ formId, page, userEmail, onMessage }) {
    const [viewers, setViewers] = R.useState([]);
    const [isConnected, setConnected] = R.useState(false);
    const callback = R.useRef(onMessage);
      ...
      const connect = () => {
        ...
        ws.onmessage = (event) => {
          ...
          try {
            ...
            if (data.type === "presence_update" && data.form_id === formId) setViewers(data.viewers || []);
        ...
        ws.onclose = () => {
          ...
          retry = setTimeout(connect, Math.min(1500 * 2 ** (failures - 1), 15e3));
        };
        ws.onerror = () => ws.close();
      };
      const resume = () => {
