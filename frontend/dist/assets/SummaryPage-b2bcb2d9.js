...
`));let d="";(z=t==null?void 0:t.disagreements)!=null&&z.length&&(d=t.disagreements.map(x=>{var A;const j=((A=x.severity)==null?void 0:A.toLowerCase())||"medium",C=x.positions.map(N=>{const b=N.experts.map(S=>i[S]||`Expert ${S}`).join(", ");return`<div class="position-block">
...
`));let v="";($=t==null?void 0:t.nuances)!=null&&$.length&&(v=t.nuances.map(x=>{const j=x.relevant_experts.map(C=>i[C]||`Expert ${C}`).join(", ");return`<div class="finding-card nuance">
        <div class="finding-header"><span class="finding-type">Nuance</span></div>
        <h4>${E(x.claim)}</h4>
        <p>${E(x.context)}</p>
        <p class="experts-line">Relevant experts: ${E(j)}</p>
      </div>`}).join(`
`));let a="";(I=t==null?void 0:t.follow_up_probes)!=null&&I.length&&(a='<ol class="probes-list">'+t.follow_up_probes.map(x=>{const j=x.target_experts.map(C=>i[C]||`Expert ${C}`).join(", ");return`<li>
      ...
      </li>`}).join("")+"</ol>");const o=n.map(x=>{const j=x.convergence_score!=null?`${(x.convergence_score*100).toFixed(0)}%`:"—";return`<tr><td>Round ${x.round_number}</td><td>${x.response_count??"—"}</td><td>${j}</td><td>${x.questions.length}</td></tr>`}).join("");let u="";(w=t==null?void 0:t.emergent_insights)!=null&&w.length&&(u=t.emergent_insights.map(x=>typeof x=="string"?`<li>${E(x)}</li>`:`<li><strong>${E(x.title||"Insight")}:</strong> ${E(x.description||"")}${x.supporting_evidence?`<br/><em>Evidence: ${E(x.supporting_evidence)}</em>`:""}</li>`).join(""),u=`<ul class="insights-list">${u}</ul>`);let f="";return t!=null&&t.confidence_map&&Object.keys(t.confidence_map).length>0&&(f=Object.entries(t.confidence_map).map(([x,j])=>{const C=(j*100).toFixed(0);return`<div class="confidence-bar-row">
...
`)}
  ...
  <table class="govuk-table">
    ...
    <tbody>${Object.entries(i).map(([x,j])=>`<tr><td class="govuk-table__cell">Expert ${E(x)}</td><td class="govuk-table__cell">${E(j)}</td></tr>`).join("")}</tbody>
  </table>`:""}
