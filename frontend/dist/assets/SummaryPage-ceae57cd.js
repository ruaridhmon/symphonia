...
`));let d="";(z=t==null?void 0:t.disagreements)!=null&&z.length&&(d=t.disagreements.map(h=>{var A;const j=((A=h.severity)==null?void 0:A.toLowerCase())||"medium",S=h.positions.map(N=>{const y=N.experts.map(C=>a[C]||`Expert ${C}`).join(", ");return`<div class="position-block">
...
`));let f="";($=t==null?void 0:t.nuances)!=null&&$.length&&(f=t.nuances.map(h=>{const j=h.relevant_experts.map(S=>a[S]||`Expert ${S}`).join(", ");return`<div class="finding-card nuance">
        <div class="finding-header"><span class="finding-type">Nuance</span></div>
        <h4>${E(h.claim)}</h4>
        <p>${E(h.context)}</p>
        <p class="experts-line">Relevant experts: ${E(j)}</p>
      </div>`}).join(`
`));let i="";(F=t==null?void 0:t.follow_up_probes)!=null&&F.length&&(i='<ol class="probes-list">'+t.follow_up_probes.map(h=>{const j=h.target_experts.map(S=>a[S]||`Expert ${S}`).join(", ");return`<li>
