...
export function createResponseWorkspace(R:typeof React,Editor:React.ComponentType<any>,remove?:(id:number)=>Promise<unknown>){
 ...
 return function ResponseWorkspace(p:Props){
  ...
  R.useEffect(()=>{
   if(current){container.current?.querySelector<HTMLElement>('.rw-reader-title')?.focus();container.current?.scrollIntoView?.({block:'start'});wasReading.current=true;}
   else if(wasReading.current){container.current?.querySelector<HTMLElement>(`[data-response-id="${returnId.current}"]`)?.focus({preventScroll:true});window.scrollTo?.({top:listScroll.current});wasReading.current=false;}
  },[active]);
  const allowLeave=()=>!container.current?.querySelector('textarea')||window.confirm('Discard unsaved response edits?');
  const open=(id:number)=>{if(!allowLeave())return;if(active===null)listScroll.current=window.scrollY;returnId.current=id;setActive(id);};
  ...
  return h('section',{className:'response-workspace',ref:container,'aria-label':'Expert responses'},
   current?h('div',{className:'rw-reader'},
    h('nav',{className:'rw-reader-nav','aria-label':'Response navigation'},button('← All responses',()=>{if(allowLeave())setActive(null);}),h('div',null,h('span',{className:'rw-position'},`${index+1} of ${filtered.length}`),button('Previous',()=>open(filtered[index-1].response.id),{disabled:index===0}),button('Next',()=>open(filtered[index+1].response.id),{disabled:index===filtered.length-1}))),
    h('header',{className:'rw-reader-heading'},h('h2',{className:'rw-reader-title',tabIndex:-1},current.name),h('p',null,`Round ${current.round.round_number}${timestamp(current.response.timestamp)?' · '+timestamp(current.response.timestamp):''}`)),
    h(Editor,{key:current.response.id,response:current.response,questions:current.questions,token:p.token,onUpdated:(response:any)=>p.onResponseUpdated(current.round.id,response)})
   ):h(R.Fragment,null,
    ...
    h('div',{className:'rw-toolbar'},h('label',null,h('span',{className:'rw-label'},'Search responses'),h('input',{type:'search',value:query,placeholder:'Search people or answers',onChange:(e:React.ChangeEvent<HTMLInputElement>)=>setQuery(e.target.value)})),h('label',null,h('span',{className:'rw-label'},'Round'),h('select',{value:roundId,onChange:(e:React.ChangeEvent<HTMLSelectElement>)=>setRoundId(e.target.value==='all'?'all':Number(e.target.value))},h('option',{value:'all'},'All rounds'),...p.structuredRounds.map(r=>h('option',{key:r.id,value:r.id},`Round ${r.round_number}`))))),
