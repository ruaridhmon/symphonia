...
export function createResponseWorkspace(R:typeof React,Editor:React.ComponentType<any>,remove?:(id:number)=>Promise<unknown>){
 ...
 return function ResponseWorkspace(p:Props){
  ...
  return h('section',{className:'response-workspace response-panel','aria-label':'Expert responses',ref:container},
    ...
    mode!=='person'&&titles.length?h('div',{className:'rp-question-bar'},h('label',null,h('span',null,'Question'),h('select',{'aria-label':'Question or claim',value:chosen,onChange:(e:React.ChangeEvent<HTMLSelectElement>)=>{if(allowLeave()){setQuestion(e.target.value);setActive(null);}}},...titles.map(title=>h('option',{key:title,value:title},title)))),h('h3',null,chosen)):null,
