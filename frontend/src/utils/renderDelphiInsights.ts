...
export function renderDelphiInsights(root:HTMLElement, round:Round, rounds:Round[], responses:RoundWithResponses[], refresh?:()=>void) {
  ...
  ['All claims',...cats].forEach(label=>{const b=button(label,()=>{root.dataset.filter=label;renderDelphiInsights(root,round,rounds,responses,refresh);});b.setAttribute('aria-pressed',String(filter===label));filters.append(b);});root.append(filters);
  ...
  selected.forEach((row)=>{
    ...
    if(row.history.filter(h=>h.n>0).length>1) {
      ...
      row.history.filter(h=>h.n>0).forEach((h,i)=>{if(i)trend.append(node('span','→','di-arrow'));trend.append(node('span',`R${h.round} ${Math.round(h.percent!)}%`));});
    ...
    const summary=node('summary','Read reasons & changes');detail.append(summary);
