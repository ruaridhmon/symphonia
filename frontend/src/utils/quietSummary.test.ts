...
it('keeps the synthesis visible when it is the only result or an existing edit action opens it',()=>{
 ...
 main.querySelector('.di-claim')!.remove();quietSummary(main);expect(card.hidden).toBe(false);expect(main.querySelector<HTMLElement>('.summary-switch')!.hidden).toBe(true);
});
