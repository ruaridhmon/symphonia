...
export type Draft={title:string;description:string;questions:ConfigurableQuestion[];allowJoin:boolean;allowPublicResponses:boolean;requireConsent:boolean;consentText:string;consentDocument:string};
...
export function FormCanvas({edit}:{edit?:CanvasEdit}){
 ...
 return <main className="form-canvas" ref={canvas}>
  <header className="fc-toolbar"><button type="button" onClick={()=>{if(!edit||!dirty||window.confirm('Leave without saving your changes?'))navigate(edit?`/admin/form/${edit.id}/summary`:'/');}}>← {edit?'Summary':'Consultations'}</button><span role="status" className="fc-save-state">{edit?(dirty?'Unsaved changes':saved||'All changes saved'):saved}</span><div><button type="button" aria-pressed={preview} onClick={()=>setPreview(!preview)}>{preview?'Edit form':'View as participant'}</button><button type="button" aria-expanded={settings} onClick={()=>setSettings(!settings)}>Settings</button><button type="button" className="cw-primary" disabled={saving||(!!edit&&!dirty)} onClick={create}>{saving?(edit?'Saving…':'Creating…'):(edit?'Save changes':'Create consultation')}</button></div></header>
  ...
  {settings?<aside className="fc-settings" aria-label="Consultation settings"><div className="fc-settings-title"><h2>Consultation settings</h2><button type="button" onClick={()=>setSettings(false)}>Close</button></div>
   ...
   {!edit?.locked?<details><summary>Import questions</summary><QuestionnaireImporter onQuestionsImported={questions=>{change({questions});setAnswers({});setSelected(0);}} onImported={result=>setImportNotice(`Imported ${result.questions.length} questions. ${result.warnings.join(' ')}`)}/>{importNotice?<p role="status">{importNotice}</p>:null}</details>:null}
   {!edit?<button type="button" onClick={()=>setLegacy(true)}>Templates and document editor →</button>:null}
  </aside>:null}
  <div className="fc-paper" data-preview={preview}>
   <p className="fc-eyebrow">{preview?'Participant view · answers here are for testing only':edit?`Round ${edit.roundNumber} · Edit questions`:'Opening round'}</p>
   <div className="fc-introduction">{preview?<><h1>{draft.title||'Untitled consultation'}</h1>{draft.description?<p>{draft.description}</p>:null}</>:<><textarea aria-label="Consultation title" className="fc-title" rows={1} placeholder="Untitled consultation" value={draft.title} onChange={e=>change({title:e.target.value})}/>{!edit?<textarea aria-label="Introduction" className="fc-description" rows={1} placeholder="Add a short introduction for your participants…" value={draft.description} onChange={e=>change({description:e.target.value})}/>:null}</>}</div>
   ...
   {!preview&&!edit?.locked?<button type="button" className="fc-add" onClick={add}>+ Add question</button>:preview?<p className="fc-preview-note">This is a preview. Test answers are not submitted or included in results.</p>:null}
  </div>
 </main>;
}
