...
export function FormCanvas({edit}:{edit?:CanvasEdit}){
 ...
 return <main className="form-canvas fc-composer" ref={canvas}>
  <header className="fc-toolbar"><button type="button" onClick={()=>{if(!edit||!dirty||window.confirm('Leave without saving your changes?'))navigate(edit?`/admin/form/${edit.id}/summary`:'/');}}>← {edit?'Summary':'Consultations'}</button><span role="status" className="fc-save-state">{edit?(dirty?'Unsaved changes':saved||'All changes saved'):saved}</span><div><button type="button" aria-pressed={preview} onClick={()=>{setSelected(null);setPreview(!preview);}}>{preview?'Edit':'Preview'}</button><button type="button" aria-expanded={settings} onClick={()=>setSettings(!settings)}>Settings</button><button type="button" className="cw-primary" disabled={saving||(!!edit&&!dirty)} onClick={create}>{saving?(edit?'Saving…':'Creating…'):(edit?'Save changes':'Create consultation')}</button></div></header>
  ...
  <div className="fc-paper" data-preview={preview}>
   <p className="fc-eyebrow">{preview?'Preview · test answers are not submitted':edit?`Round ${edit.roundNumber}`:'New consultation'}</p>

   <div className="fc-introduction">{preview?<><h1>{draft.title||'Untitled consultation'}</h1><p className={!draft.description?'fc-empty-intro':undefined}>{draft.description||'Add a short introduction for your participants…'}</p></>:<><label className="sr-only" htmlFor="canvas-title">Consultation title</label><textarea id="canvas-title" aria-label="Consultation title" className="fc-title" rows={1} placeholder="Name your consultation" value={draft.title} onChange={e=>change({title:e.target.value})}/>{!edit?<textarea aria-label="Introduction" className="fc-description" rows={1} placeholder="Add an introduction (optional)" value={draft.description} onChange={e=>change({description:e.target.value})}/>:null}</>}</div>
   ...
   {documentMode?<div className="fc-document">{preview||edit?.locked?<DocumentTemplateResponse template={template} answers={answers} onChange={(key,value)=>setAnswers(previous=>({...previous,[key]:value}))}/>:<DocumentTemplateEditor value={template} onChange={documentTemplate=>{change({documentTemplate});setAnswers(buildInitialDocumentTemplateResponses(documentTemplate));}} previewAnswers={answers} onPreviewChange={(key,value)=>setAnswers(previous=>({...previous,[key]:value}))}/>}</div>:draft.questions.map((q,i)=>isVisible(q)?<section key={i} data-preview={preview} className={`fc-question ${selected===i?'fc-selected':''}`}>
    ...
    <div className="fc-answer" hidden={!preview&&!q.label.trim()} aria-label={`Answer to question ${i+1}`}>
