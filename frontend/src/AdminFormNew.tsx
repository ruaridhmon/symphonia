...
export function FormCanvas({edit}:{edit?:CanvasEdit}){
 ...
 return <main className="form-canvas" ref={canvas}>
  ...
  <div className="fc-paper" data-preview={preview}>
   ...
   <div className="fc-introduction">{preview?<><h1>{draft.title||'Untitled consultation'}</h1><p className={!draft.description?'fc-empty-intro':undefined}>{draft.description||'Add a short introduction for your participants…'}</p></>:<><label className="sr-only" htmlFor="canvas-title">Consultation title</label><textarea id="canvas-title" aria-label="Consultation title" className="fc-title" rows={1} placeholder="What is your consultation about?" value={draft.title} onChange={e=>change({title:e.target.value})}/>{!edit?<textarea aria-label="Introduction" className="fc-description" rows={1} placeholder="Add a short introduction for your participants…" value={draft.description} onChange={e=>change({description:e.target.value})}/>:null}</>}</div>
