...
let html=fs.readFileSync('dist/index.html','utf8').replace(/<script type="importmap">(.*?)<\/script>/,(_,json)=>{const map=JSON.parse(json);map.imports['/assets/AdminFormNew-C6eEpVHb.js']='/assets/AdminFormNew-canvas-v5.js';map.imports['/assets/FormEditor-DIpaScmC.js']='/assets/FormEditor-canvas-v3.js';return '<script type="importmap">'+JSON.stringify(map)+'</script>';});
