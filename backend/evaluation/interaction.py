...
def experiment(client,w,panel,model,mode,participant_model,words=500):
    prefix=f'{w["map_id"]}/B/{mode}/{participant_model}'
    ...
    for p in panel['people']:
        ...
        if mode=='controlled':
            ...
            for row in rows:
                ...
                row['reason']='Controlled prior odds 1, updated once per independent available source.'
    ...
    for arm in ('symphonia','exact','structured','none'):
        ...
        def respond(pair):
            ...
            computed=probabilities(w,private_ids|exposed,selected) if mode=='controlled' else None
        ...
        arms.append({'arm':arm,'feedback':shown,'initial':initial,'people':people,'reference':final_table,'output':output,'scores':labels,'decisions':readers,'initial_brier':initial_brier,'final_brier':final_brier})
    return client.store.put('interactive/'+digest(prefix)+'.json',{'map_id':w['map_id'],'policy_setting':w['policy_setting'],'mode':mode,'participant_model':participant_model,'n':len(panel['people']),'arms':arms})
