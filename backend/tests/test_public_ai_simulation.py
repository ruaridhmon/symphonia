...
FIXTURE = json.loads((Path(__file__).parent / 'fixtures/public_ai_delphi.json').read_text())
...
def questions():
    return [q for i,claim in enumerate(FIXTURE['claims'],1) for q in [
        ...
        {'questionId':f'claim_{i}_comment','sectionTitle':f'Claim {i}: {claim}','label':'Comments or clarification','inputType':'textarea','optional':True}]]
...
def test_full_authenticated_simulation(client, admin_headers):
    form=create_form(client,admin_headers,title=FIXTURE['title'],questions=[{'questionId':'proposal','label':'Where should humans stay in control?','inputType':'textarea','optional':False}])
    form_id=form['id']
    participants=[]
    for i,e in enumerate(FIXTURE['experts']):
        token=register_and_login(client,f'public-ai-{i}@example.com')
